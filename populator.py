...
programs.sort()
last_infectable_file = programs[len(programs) - 1]
last_infectable_file_num = int(last_infectable_file[-4])

new_infectible_nums = [*range(last_infectable_file_num + 1, last_infectable_file_num + num_new_infectibles)]

for num in new_infectible_nums:
    new_file_name = 'infect_me_' + str(num) + '.py'
    new_file = open('petri_dish_1/' + new_file_name, 'w')
